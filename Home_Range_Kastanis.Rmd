...
```{r files}
files <- list.files(path = ".", pattern = "[Gray Wolf]+[0-9]+", full.names = TRUE)
```
...
autoplot.OpenStreetMap(raster_utm, expand = TRUE) + theme_bw() +
  ...
  geom_point(data=data, aes(location.long,location.lat,
...
```{r raster_function}
mcp_raster <- function(individual.local.identifier){
  data <- read.csv(individual.local.identifier)
